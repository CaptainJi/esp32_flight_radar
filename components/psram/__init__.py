...
async def to_code(config):
    ...
    if config[CONF_MODE] == TYPE_OCTAL and speed == 120:
        ...
        # === 本地覆寫:相對於上游 ESPHome 唯一的改動 ===
        # 上游在此無條件開啟 CONFIG_SPIRAM_TIMING_TUNING_POINT_VIA_TEMPERATURE_SENSOR
        # (IDF >= 5.4)。但 IDF 的 psram_adjust_timing_point_via_tsens() 只支援
        # flash 廠商 ID 0xC8(GigaDevice)/0x20(XMC),其他 flash 的板子會在
        # 系統初始化階段回傳 ESP_ERR_NOT_SUPPORTED(0x106)→ abort → 無限重開機。
        # Waveshare ESP32-S3-Touch-LCD-5B 的 flash 不在名單內,故不開啟此選項
        # (IDF 自己的 Kconfig 預設也是 n,只失去溫度漂移的即時補償)。
        # 這樣才能保住 120MHz octal PSRAM 的頻寬,讓 1024x600 面板跑 60Hz。
