...
#include "esp_heap_caps.h"   // 本地覆寫:繪圖緩衝改配內部 SRAM(見 setup())
...
void LvglComponent::setup() {
  ...
  void *buffer = nullptr;
  // === 本地覆寫:相對於上游 ESPHome 2026.3.3 唯一的改動 ===
  // 上游此處用 malloc(),但本板 CONFIG_SPIRAM_USE_MALLOC=y 會把大配置導去
  // PSRAM——繪圖緩衝落在 PSRAM 時,LVGL 渲染流量與 RGB 面板掃描 DMA
  // (~56MB/s)搶同一條 PSRAM 匯流排,實測 lvgl 單次操作 400~1200ms,並加劇
  // GDMA underrun(畫面抖動/水平錯置)。改成明確要求內部 SRAM:渲染期間完全
  // 不佔 PSRAM 頻寬,只在 flush 時寫一次 framebuffer(Seeed round-display
  // workshop 實測同款做法 25→30FPS,結論「記憶體頻寬比計算週期重要」)。
  // 配 buffer_size: 12%(frac=8)緩衝為 1024*600/8*2 ≈ 154KB;內部 RAM 靜態
  // 僅用 ~14%,放得下。要不到內部 SRAM 時照舊回退 PSRAM,不會開不了機。
  // 再減半(frac 8→16,154KB→77KB)。三種大小的實測結論:
  //   1/8(154KB):渲染最快(慢操作僅 1 次),但餓死 TLS——mbedtls 動態緩衝
  //     alloc(4770) 失敗、握手 -0x7F00,資料全抓不到(小配置受
  //     SPIRAM_MALLOC_ALWAYSINTERNAL 門檻限制必走內部 RAM,退無可退)。
  //   1/16(77KB):TLS 健康、渲染接近最佳(238~355ms 偶發)← 採用
  //   1/32(38KB):TLS 健康,但整屏切 32 段 flush 的開銷反而讓慢操作暴增
  //     (464~861ms、8 次/180s,比不覆寫還糟),縮過頭。
  if (this->buffer_frac_ >= MIN_BUFFER_FRAC / 2) {
    buffer = heap_caps_malloc(buf_bytes / 2, MALLOC_CAP_INTERNAL | MALLOC_CAP_8BIT);  // NOLINT
    if (buffer != nullptr) {
      frac *= 2;
      buffer_pixels /= 2;
      buf_bytes /= 2;
  ...
  if (buffer != nullptr) {
    ESP_LOGI(TAG, "draw buffer %zu bytes in internal SRAM (1/%zu screen)", buf_bytes, frac);
  } else {
