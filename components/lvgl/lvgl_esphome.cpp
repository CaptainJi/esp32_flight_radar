...
void LvglComponent::setup() {
  ...
  void *buffer = nullptr;
  // === 本地覆寫:相對於上游 ESPHome 2026.6.5 唯一的改動 ===
  // 上游已經會「先要內部 SRAM、失敗才退 PSRAM」(這正是 LVGL 8 分支那份覆寫的
  // 目的,故已無須重做),但 buffer_size 被量化成 1/1、1/2、1/4、1/8,最小只能
  // 到 1/8。1024x600 的 1/8 = 154KB 內部 SRAM,實測會把內部 RAM 榨乾:
  //   Dynamic Impl: alloc(4770 bytes) failed → mbedtls 握手 -0x7F00
  // 把 TLS 記憶體改配 PSRAM(CONFIG_MBEDTLS_EXTERNAL_MEM_ALLOC)只是把問題推給
  // 下一位:AES 硬體加速器遇到 PSRAM 來源時要另配「內部 DMA 跳板緩衝」,同樣
  // 配不出來 → esp-aes: Failed to allocate memory → TLS 讀取 -0x0001。
  // 所以這裡照 LVGL 8 分支的結論再減半一次:1/8 → 1/16(77KB)。該分支的實測
  // 取捨是 154KB 渲染最快但餓死 TLS、77KB 兩者兼得(渲染只差一點)、38KB 則
  // 因 32 段 flush 的開銷反而更慢。要不到內部 SRAM 時照舊回退 PSRAM。
  if (this->buffer_frac_ >= MIN_BUFFER_FRAC / 2) {
    buffer = lv_alloc_draw_buf(buf_bytes / 2, true);  // NOLINT
    if (buffer != nullptr) {
      frac *= 2;
      buf_bytes /= 2;
      ESP_LOGI(TAG, "draw buffer %zu bytes in internal SRAM (1/%d screen)", (size_t) buf_bytes, (int) frac);
    }
  }
  if (buffer == nullptr) {
    buffer = lv_alloc_draw_buf(buf_bytes, false);  // NOLINT
    ESP_LOGW(TAG, "internal SRAM draw buffer unavailable, falling back to PSRAM");
  }
...
static void *lv_alloc_draw_buf(size_t size, bool internal) {
  ...
  // 本地覆寫:上游的 internal 只給 MALLOC_CAP_8BIT,而 CONFIG_SPIRAM_USE_MALLOC=y
  // 時這個 cap 也涵蓋 PSRAM(只是內部堆積優先),要不到內部 RAM 也不會失敗、
  // 會靜靜落在 PSRAM。這裡改成明確要求 MALLOC_CAP_INTERNAL,讓上面的
  // 「內部 SRAM / 退回 PSRAM」判斷與 log 反映真實情況。
  buffer = heap_caps_aligned_alloc(LV_DRAW_BUF_ALIGN, size,
                                   internal ? (MALLOC_CAP_INTERNAL | MALLOC_CAP_8BIT) : cap_bits);  // NOLINT
  if (buffer == nullptr)
