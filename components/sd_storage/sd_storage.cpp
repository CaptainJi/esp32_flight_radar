...
// 上游從不設定 host.pwr_ctrl_handle。在 ESP32-P4 上,SD 卡的 IO 電軌是由晶片內部
// LDO 供電的,不打開就沒電,卡永遠不會回應(掛載重試三次後失敗)。
// Espressif 官方 BSP 的做法見 esp-bsp/bsp/esp32_p4_function_ev_board/
// esp32_p4_function_ev_board.c:199 —— 建立 on-chip LDO 電源控制再掛到 host 上。
// 通道編號在 P4 Function EV Board 與沿用其接線的板子(含 Waveshare 7B)上是 4;
// 板子不同就用 build_flags 覆寫 -DSD_STORAGE_P4_LDO_CHAN=<n>。
#include "sd_pwr_ctrl_by_on_chip_ldo.h"
#ifndef SD_STORAGE_P4_LDO_CHAN
#define SD_STORAGE_P4_LDO_CHAN 4
#endif
#endif
...
#if defined(USE_ESP32_VARIANT_ESP32P4)
static sd_pwr_ctrl_handle_t s_pwr_ctrl = nullptr;   // 只建立一次,unmount/remount 共用
#endif
...
#if defined(USE_ESP32_VARIANT_ESP32P4)
  // 本地覆寫(見檔首):P4 的 SD IO 電軌要先把晶片內部 LDO 打開,否則卡不會回應。
  // 必須在下面的 sdmmc_host_init_slot() 之前;host 是傳參考給 mount_manual_(),
  // 所以兩條掛載路徑都吃得到。
  if (s_pwr_ctrl == nullptr) {
    sd_pwr_ctrl_ldo_config_t ldo_config = {};
    ldo_config.ldo_chan_id = SD_STORAGE_P4_LDO_CHAN;
    esp_err_t ldo_ret = sd_pwr_ctrl_new_on_chip_ldo(&ldo_config, &s_pwr_ctrl);
    if (ldo_ret != ESP_OK) {
      // 不直接失敗:若某塊板子的 SD 電軌是外部固定供電,沒有 LDO 也可能掛得起來。
      ESP_LOGW(TAG, "on-chip LDO ch%d unavailable: %s (SD may not power up)", SD_STORAGE_P4_LDO_CHAN,
               esp_err_to_name(ldo_ret));
      s_pwr_ctrl = nullptr;
    } else {
      ESP_LOGD(TAG, "  SD IO powered by on-chip LDO channel %d", SD_STORAGE_P4_LDO_CHAN);
    }
  }
