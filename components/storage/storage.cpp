...
#include <cerrno>
#include <cstring>
#include <vector>   // local change #2 in unregister_storage(), see below
#ifdef USE_ESP32
...
void StorageRegistry::unregister_storage(Storage *s) {
...

  {
    ...
    // Local change #2 (see the header of __init__.py): stock ESPHome's FixedVector has no
    // pop_back() -- the fork adds it in core/helpers.h, which this project does not vendor.
    // Copy out (FixedVector converts to std::vector), drop the tail, and refill. clear()
    // keeps the allocation, so the swap-remove above is still reallocation-free as intended;
    // the one temporary lives on an unregister path that runs a handful of times at most.
    std::vector<Storage *> survivors = this->storages_;
    survivors.pop_back();
    this->storages_.clear();
    for (auto *item : survivors)
      this->storages_.push_back(item);
  }
