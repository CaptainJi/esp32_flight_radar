# Vendored from p1ngb4ck/esphome @ d15c20ded79e13c9021e8d1e0749ef8e389ba86b
# (branch `storage-testing`, 2026-08-02) -- esphome/components/storage/.
#
# This fork patches ESPHome *core* in places, and we deliberately vendor only these two
# component directories -- so each core dependency needs a local shim. Two so far:
#
#   #1  _esp32_exfat_enabled() below -- the fork adds `enable_exfat` to the core esp32
#       component; that import is allowed to fail, leaving the filesystem at FAT32.
#   #2  storage.cpp, unregister_storage() -- the fork adds FixedVector::pop_back() to
#       core/helpers.h; rebuilt without it.
#
# (A third dependency is not a code change: sd_storage reads CONFIG_FATFS_MAX_LFN, which
#  only exists when FATFS long filenames are on. The fork's esp32 component defaults it;
#  we set it in the board file's sdkconfig_options instead.)
#
# Search for "Local change" in both directories to find them all.
#
# Why `storage-testing` and not `dev`: on `dev` the two components are out of sync --
# sd_storage imports request_storage_device(), which does not exist in that commit's
# storage/__init__.py, so the config fails to load. `storage-testing` is the branch the
# author's own "max config" reference was generated against, and there they agree.
#
# Why vendored rather than `external_components: source: github://...@<branch>`:
# both are moving branches, so an unpinned reference would silently change the firmware
# between builds. sd_storage AUTO_LOADs this component, so both must come from one commit.
#
# Note this only builds against a recent ESPHome: sd_storage imports VARIANT_ESP32S31,
# which does not exist in 2026.3.3 (the `main` branch env). The P4 SD support therefore
# lives on the `lvgl9` branch only -- see PLAN_P4_SD.md.
...
def _esp32_exfat_enabled(fconf) -> bool:
    # Local change #1 (see the header of this file): `enable_exfat` is an addition this
    # fork makes to the *core* esp32 component, so the import fails on stock ESPHome.
    # Vendoring the fork's esp32 component to satisfy it is out of the question -- it is
    # a large, version-sensitive core component and this project already carries its own
    # psram/lvgl overrides that assume the stock one. Absent the option, exFAT simply is
    # not compiled in and the filesystem is always FAT32, which is exactly what "False"
    # means to both callers below. FAT32 is all this project needs.
    try:
