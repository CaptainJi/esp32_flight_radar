...
void I2SAudioSpeakerBase::loop() {
  ...
  // Soft overflow (Tab5/P4): drop oldest event, do not restart task (see i2s_on_sent_cb)
  if (event_group_bits & SpeakerEventGroupBits::ERR_DROPPED_EVENT) {
    // Rate-limit: only warn once until cleared; bit cleared here so continuous overflow won't spam
...
bool IRAM_ATTR I2SAudioSpeakerBase::i2s_on_sent_cb(i2s_chan_handle_t handle, i2s_event_data_t *event, void *user_ctx) {
  ...
  if (xQueueIsQueueFullFromISR(this_speaker->i2s_event_queue_)) {
    // Queue full: discard oldest completion timestamp and continue (2025.x behavior).
    // Stock 2026.5+ also sets COMMAND_STOP and kills the task — on ESP32-P4 + LVGL that
    // fires during short RTTTL beeps and voice clips. Soft-flag only; timestamps may drift
    // slightly but playback keeps going. Primary mitigation: larger I2S_EVENT_QUEUE_COUNT.
    int64_t dummy;
