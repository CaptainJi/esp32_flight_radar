...
#endif
// Sized to absorb scheduling jitter on P4 + LVGL (stock *2 is ~100ms; we keep ~800ms headroom).
// Overflow is soft-recovered in i2s_on_sent_cb (drop oldest event+record, no task restart).
static constexpr size_t I2S_EVENT_QUEUE_COUNT = DMA_BUFFERS_COUNT * 16;
    ...
    while (this->pause_state_ || !this->timeout_.has_value() ||
      ...
      while (xQueueReceive(this->i2s_event_queue_, &write_timestamp, 0)) {
        ...
        if (xQueueReceive(this->write_records_queue_, &real_frames, 0) != pdTRUE) {
          // Soft-dropped ISR events can leave an unpaired timestamp; skip it instead of
          // fatal ERR_LOCKSTEP_DESYNC restart (P4 + LVGL / RTTTL regression #16656 class).
      ...
      if (xQueueSend(this->write_records_queue_, &real_frames_in_buffer, 0) != pdTRUE) {
        // Soft recovery: drop oldest record (may pair with a soft-dropped ISR event) and retry.
        uint32_t dropped = 0;
        if (xQueueReceive(this->write_records_queue_, &dropped, 0) == pdTRUE &&
            xQueueSend(this->write_records_queue_, &real_frames_in_buffer, 0) == pdTRUE) {
          if (dropped > 0 && pending_real_buffers > 0) {
            pending_real_buffers--;
          }
          ESP_LOGW(TAG, "Write records queue full (dropped oldest, continuing)");
        } else {
