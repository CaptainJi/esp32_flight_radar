...
## 加速:回波混合只走有降雨的列(**兩個分支都有**)

底圖有三張 PSRAM 緩衝,分工要先分清楚,才知道時間花在哪:

| 緩衝 | 內容 | 何時重畫 | 570² 大小 |
|---|---|---|---|
| `cache` | 底色 + 地圖輪廓 | **只有** lat/lon/range/MAP 變動時 | 634 KB |
| `g_echo_buf` | 回波 RGB565 + alpha(3 B/px) | 每 10 分鐘背景重抓 | 952 KB |
| canvas draw buf | 合成結果(螢幕上那張) | 每次 `radar_rebuild_base` | 634 KB |

所以**每 10 分鐘那條路徑不畫地圖輪廓**(`fresh` 就 memcpy 快取),真正的固定成本是
memcpy + 回波混合 + 十字線/距離環 + 全部 ATC 圖層。其中回波混合原本是整張
324,900 像素逐一掃過、每像素讀 3 bytes PSRAM ≈ 950KB,**就算完全沒下雨也照掃**。

改法:合成磚圖時順手記下每一列非透明像素的左右界(`g_echo_x0[]` / `g_echo_x1[]`,
共 2.2KB),混合就只走那些區段;空白列連一次 PSRAM 讀取都不必。降雨通常只佔畫面
一小塊,沒下雨時整張全空 → 幾乎歸零。

這個改動與 LVGL 版本無關,所以 **`main` 與 `lvgl9` 都做了**(main 是 commit
`4d2b6ae`,lvgl9 是同一份 cherry-pick)。比較兩個分支時這一項是對齊的。
